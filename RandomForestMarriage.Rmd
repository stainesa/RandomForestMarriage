...
rm(Tree_Prediction_0.01, Tree_Prediction_0.02, Tree_Prediction_0.03)
...
rm(DATA_test, DATA_train, TEST, TRAIN)
...
TRAIN <- Attrition %>% filter(Training)
TEST  <- Attrition %>% filter(!Training)
```
...
# number of features
n_features <- length(setdiff(names(Attrition), "Attrition")) - 2
...
attr_rf1 <- ranger(
  ...
  RF_data_train_2 = bind_cols(NoisyAttrition = Working$BiassedAttrition,
                    Prediction = predictions_train[,2])
