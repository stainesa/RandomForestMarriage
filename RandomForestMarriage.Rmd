...
First, this show the enormous difference between the results on training data and test data, with no overlap between the prediction for the test and training data. The test data, of course, represents something like the real world performance. This is true for all error rates,including zero-error.

As the error rate increase, the prediction of the training data, trained on the noisy data hardly shifts at all. By comparison the prediction of the true data from the noisy data gets a bit worse for the training sample.

For the test data, the overall prediction level is poor, but the gap between the predictions for noisy data and the predictions of the truth rises very steeply as the error rate increases. It also becomes much more variable (represented by the width of each boxplot) as the level of noise increases.
...
A similar analysis for the variable Gender, which is one of the lowest importance variable in the whole data set (model-based importance of 0.3) confirms this hypothesis, and demonstrates some of the range of effects of bias on performance.
