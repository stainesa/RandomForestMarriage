...
date: "`r Sys.Date()`"
bibliography: "RandomForestMarriage.bib"
output:
...
# Introduction - why examine Random FCould not export bibliography: no selectionorest models in particular?
...
While the full model does much better on the training data than the simpler models, on the test data its performance is no better, and none of these classifiers would be regarded as very effective in practice. There are a number of ways to improve the performance of these trees, notably cross-validation, more careful variabel selection, and a range of methods for improving balance. Any introduction to machine learning would cover these extensively, for example @jamesIntroductionStatisticalLearning2021. However, for this analysis, following these important ideas further would distract from the focus on the social implications of the use of the Home Office model. 
...
OneBiassedModel <- function(Data, Outcome, Source_of_bias, Level_of_bias = 0.1){
  # number of features
  n_features <- length(names(Data)) - 2 # Training and Attrition
...
attr_rf1 <- ranger(
  BiassedAttrition ~ ., 
  data = TRAIN  %>% select(-{{ Outcome }}, -{{ Source_of_bias }}, -Training), 
  num.trees = 1000,
  mtry = floor(n_features / 3),
  probability=TRUE,
  importance = 'impurity',
...
OneBiassedModel(Data = Attrition, Outcome = Attrition,
