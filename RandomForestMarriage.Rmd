...
ggsave("Images/Bias_EnvironmentSatisfaction.png")
ggsave("Images/Bias_EnvironmentSatisfaction.pdf")
```
...
#ModelBiassedSims_Gender <- seq(0,0.15, 0.01) %>%
#    set_names() %>%
#     map(~replicate(1000, OneBiassedModel(Data = Attrition,
#                                         Source_of_bias = Gender,
#                                          Level_of_bias = .x) ) #)
...
D <- do.call(rbind, lapply(ModelBiassedSims_Gender, data.frame)) %>%
  rownames_to_column('NAME') %>%
  mutate(Error_rate = str_split(NAME,"\\.[AUC_]", simplify = TRUE )[,1]) %>%
  mutate(Set = str_extract(NAME,"\\_(.*?)\\_")) %>%
  mutate(Set = str_replace_all(Set, "\\_", "" )) %>%
...
ggsave("Images/Bias_Gender.png")
ggsave("Images/_Gender.pdf")
```
...
Attrition$Distance = cut(Attrition$DistanceFromHome, breaks =c(0,5,9,19,41), labels =c(1,2,3,4))
...
kbl(table(Attrition$Attrition, Attrition$Distance),  booktabs = T,
    ...
    col.names = c('Left IBM?', '< 5', '5 to 9', '9 to 19', '>19')) %>%
  kable_styling(latex_options = c("repeat_header"))
...
Working <- Attrition %>%
  mutate(BiassedAttrition =
           bias_YesNo(Variable = Attrition,
                       Source_of_bias = as.numeric(Distance),
                       Level_of_bias = 0.0))
#  table(Working$Attrition, Working$BiassedAttrition)
...
attr_rf1 <- ranger(
  BiassedAttrition ~ ., 
  data = TRAIN %>% select(-Attrition, -Training, -DistanceFromHome),
  num.trees = 1000,
  mtry = floor(n_features / 3),
  probability=TRUE,
  importance = 'impurity',
...
#ModelBiassedSims_Distance <- seq(0,0.15, 0.01) %>%
#    set_names() %>%
#     map(~replicate(1000, OneBiassedModel(Data = Attrition,
#                                         Source_of_bias = Distance,
#                                          Level_of_bias = .x) ) )
...
D <- do.call(rbind, lapply(ModelBiassedSims_Distance, data.frame)) %>%
  rownames_to_column('NAME') %>%
  mutate(Error_rate = str_split(NAME,"\\.[AUC_]", simplify = TRUE )[,1]) %>%
  mutate(Set = str_extract(NAME,"\\_(.*?)\\_")) %>%
  mutate(Set = str_replace_all(Set, "\\_", "" )) %>%
...
```{r Graph the biassed simulations Distance, fig.cap= 'Simulation based estimates of the effect of increasing bias due to Distance from Home on performance, as measured by Area Under the Curve, for Random Forest models of the IBM data set.', fig.dpi=1200, fig.height=8, fig.width=8}

ggplot(data <- D,
       aes(x = AUC, y = Error_pct, group = interaction(Set,Type,Error_rate),
           colour = Set, fill = Type)) +
    geom_boxplot(outliers = TRUE, outlier.size = 0.8, position = 'dodge2') +
    labs(title = 'Impact of increasing level of bias related to Distance from Home on training and test datasets',
         x = "Area under Curve",
         y = "Simulated bias rate in training data") +
    theme_minimal() +
  scale_colour_manual(values=c("red", "black")) +
...
ggsave("Images/Bias_Distance_from_home.png")
ggsave("Images/Bias_Distance_from_home.pdf")
```
...
## TESTING
#########################################################################
## Effect of bias of a low importance variable - Gender TESTING


```{r Setup for Bias Gender_TEST}
set.seed(465465)
...
```{r Random forest bias Gender_TEST check, results = 'hide'}
set.seed(5)
# train a default random forest model on the Training data
# We have to remove the original Attrition, and the Training variables
attr_rf1 <- ranger(
...
```{r Predict from random forest bias Gender_TEST, results = 'hide'}
predict_ranger_train <- predict(attr_rf1, TRAIN)
predict_ranger_test <- predict(attr_rf1, TEST)
...
```{r Plots from random forest bias Gender_TEST, results = 'hide', eval = FALSE}
...
```{r Function for biassed simulation  Gender_TEST, results = 'hide'}
...
```{r Run the biassed simulations  Gender_TEST (SLOW)}
ModelBiassedSims_Gender_TEST <- seq(0.0,0.15, 0.025) %>%
    set_names() %>%
     map(~replicate(1000, OneBiassedModel(Data = Attrition,
                                         Source_of_bias = Attrition$Gender,
                                          Level_of_bias = .x)
...
readr::write_rds(ModelBiassedSims_Gender_TEST, file = "data/ModelBiassedSims_Gender_TEST.Rds")
ModelBiassedSims_Gender_TEST <- readr::read_rds(file = "data/ModelBiassedSims_Gender_TEST.Rds")

```

```{r Post-process the biassed simulations Gender}
DD <- do.call(rbind, lapply(ModelBiassedSims_Gender_TEST, data.frame)) %>%
  rownames_to_column('NAME') %>%
...
ggplot(data <- D %>% filter(Category == 'RR'),
       aes(y = Values, x = Error_pct, group = interaction(Set,Error_rate),
           colour = Set, fill = Set)) + geom_point() +
    labs(title = 'Impact of increasing level of bias related to Gender on test datasets',
         ...
         y = "Effect of variable") +
    theme_minimal()

ggsave("Images/Bias_Gender_TEST.png")
ggsave("Images/_Gender_TEST.pdf")
```
