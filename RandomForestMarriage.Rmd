...
```{r Plots from random forest 2, results = 'hide'}
gTree_ranger_train <- ggplot(RF_data_train, aes(d = Attrition,
                         m = Prediction)) +
    geom_roc(colour = 'blue', labelsize = 3) +
  geom_abline(color='red', slope = 1, intercept = 0)
...
```{r Random forest bias EnvironmentSatisfaction check}
set.seed(5)
# train a default random forest model on the Training data
# We have to remove the original Attrition, and the Training variables
attr_rf1 <- ranger(
...
```{r Predict from random forest bias EnvironmentSatisfaction }
predict_ranger_train <- predict(attr_rf1, TRAIN)
predict_ranger_test <- predict(attr_rf1, TEST)
...
```{r Plots from random forest bias EnvironmentSatisfaction, results = 'hide'}
gTree_ranger_train <- ggplot(RF_data_train, aes(d = Attrition,
                         m = Prediction)) +
    geom_roc(colour = 'blue', labelsize = 3) +
  geom_abline(color='red', slope = 1, intercept = 0)
...
```{r Function for biassed simulation EnvironmentSatisfaction}
...
```{r Graph the biassed simulations EnvironmentSatisfaction, fig.cap= 'Simulation based estimates of the effect of increasing bias on performance, as measured by Area Under the Curve, for Random Forest models of the IBM data set.', fig.dpi=1200, fig.height=8, fig.width=8}
...
The overall pattern here is similar to that in the previous graph of the effects of random error. Compared to the training data both sets of test data have much worse performance. However, the variability in estimates is not as high when bias is added to the data, as when random noise is added to the data. This is probably because the bias only goes one way, with certain people more likely to be misclassified as at risk of 'Attrition', when in fact they are not.
...
A similar analysis for the variable Gender, which is the lowest importance variable in the whole data set (model-based importance of 0.11) confirms this hypothesis, and demonstrates some of the range of effects of bias on performance.
...
```{r Random forest bias Gender check}
set.seed(5)
# train a default random forest model on the Training data
# We have to remove the original Attrition, and the Training variables
attr_rf1 <- ranger(
...
```{r Predict from random forest bias Gender}
predict_ranger_train <- predict(attr_rf1, TRAIN)
predict_ranger_test <- predict(attr_rf1, TEST)
...
```{r Plots from random forest bias Gender, results = 'hide'}
gTree_ranger_train <- ggplot(RF_data_train, aes(d = Attrition,
                         m = Prediction)) +
    geom_roc(colour = 'blue', labelsize = 3) +
  geom_abline(color='red', slope = 1, intercept = 0)
...
```{r Function for biassed simulation  Gender}
...
D <- do.call(rbind, lapply(ModelBiassedSims_Gender, data.frame)) %>%
  ...
  mutate(Set = str_replace_all(Set, "\\_", "" )) %>%
  mutate(Type = str_extract(NAME,"[^_]+$")) %>%
  select(NAME,Error_rate,Set,Type,X1:X100)
...
```{r Graph the biassed simulations Gender, fig.cap= 'Simulation based estimates of the effect of increasing bias on performance, as measured by Area Under the Curve, for Random Forest models of the IBM data set.', fig.dpi=1200, fig.height=8, fig.width=8}
